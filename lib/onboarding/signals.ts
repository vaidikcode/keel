...
/**
 * Fold an intake into the v3 profile so both onboarding shapes land in the
 * same place the ranking reads from.
 */
export function applyIntakeToProfile(profile: Profile, intake: Intake): Profile {
  const interests: Profile["interests"] =
    intake.vehicle === "crypto"
      ? ["crypto", "broad-funds"]
      : intake.vehicle === "trading"
        ? ["growth-tech", "crypto"]
        : intake.vehicle === "stocks"
          ? ["large-stable", "growth-tech", "dividend"]
          : ["broad-funds", "bond-cash", "large-stable"];
  ...
  const risk: Profile["risk"] =
    intake.riskBand === "high"
      ...
      : intake.riskBand === "balanced"
        ? "balanced"
        : "careful";
  const lossTolerance: Profile["lossTolerance"] =
    intake.riskBand === "high" ? 40 : intake.riskBand === "balanced" ? 20 : intake.riskBand === "low" ? 10 : 5;
  return {
    ...profile,
    interests,
    horizon,
...
/** The reverse mapping: derive an intake from a v3 profile so signals stay populated. */
export function intakeFromProfile(profile: Profile): Intake {
  const first = profile.interests[0];
  const vehicle: Intake["vehicle"] =
    first === "crypto"
      ? "crypto"
      : first === "growth-tech" || first === "large-stable" || first === "dividend"
        ? "stocks"
        : "unsure";
  const timescale: Intake["timescale"] =
    profile.horizon === "soon"
      ? "months"
      : profile.horizon === "medium"
        ? "months"
        : profile.horizon === "future"
          ? "decade"
          : "years";
  const riskBand: Intake["riskBand"] =
    profile.risk === "comfortable"
      ? "high"
      : profile.risk === "balanced"
        ? "balanced"
        : profile.lossTolerance === 5
          ? "veryLow"
          : "low";
  return {
    version: 1,
    ...
    budgetLow: profile.amount,
    budgetHigh: profile.amount,
    riskBand,
  };
}
