...
export type Contribution = { field: keyof Profile; delta: number; text: string };
export type Capacity = { score: number; contributions: Contribution[] };
export type Fit = { score: number; reasons: string[] };

const RISK: Record<Profile["risk"], number> = { careful: -20, balanced: 0, comfortable: 15, unknown: 0 };
const LOSS: Record<string, number> = { "5": -20, "10": -8, "20": 5, "40": 15, unknown: 0 };
const HORIZON: Record<Profile["horizon"], number> = { soon: -20, medium: -8, long: 5, future: 12, unknown: 0 };
const EMERGENCY: Record<Profile["emergency"], number> = { yes: 5, no: -15, unknown: 0 };
const DEBT: Record<Profile["debt"], number> = { yes: -10, no: 3, unknown: 0 };
const INCOME: Record<Profile["income"], number> = { stable: 5, variable: -5, none: -12, unknown: 0 };
// Investing experience changes how much explanation a person needs, not how
// much financial risk they can carry.
const EXPERIENCE: Record<Profile["experience"], number> = { new: 0, some: 0, experienced: 0 };
const GOAL: Record<Profile["goal"], number> = { retirement: 4, wealth: 2, explore: 0, purchase: -6 };
const INTENT: Record<Profile["intent"], number> = { learn: 0, choose: 0, understand: 0 };
