...
> Status, 13 Sep 2026: the extended eleven-step onboarding (interests, loss tolerance, income, cash buffer, debt) is what renders; it writes the v3 profile that ranking reads, and `intake`/`signals` are derived from it server-side so the five-question contract below stays populated. The five-question UI components remain in the tree for a later merge of the two designs.
